...
class KineticRequestCeUserRetrieveFromListV1
  ...
  def execute
    ...
    usernames.split(",").each{|username|
      username = URI.encode(username)
      api_route = "#{server}/app/api/v1/users/#{username}?include=details,attributes,profileAttributes"

      begin
        resource = RestClient::Resource.new(api_route, { :user => api_username, :password => api_password })
        response = resource.get
        user_details.push(JSON.parse(response)["user"])
      rescue RestClient::Exception => error
        if error.http_code == 404
          # Do nothing with the error.  Record doesn't exist which is valid in this context.
          users_not_found.push(username)
        else
    ...
    <<-RESULTS
    <results>
     <result name="User Details JSON">#{escape(user_details.to_json)}</result>
     <result name="Users Not Found JSON">#{escape(users_not_found.to_json)}</result>
    </results>
    RESULTS
