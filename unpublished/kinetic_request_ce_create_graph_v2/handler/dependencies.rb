...
# Load the ruby google charts library unless it has already been loaded.  This prevents
# multiple handlers using the same library from causing problems.
if not defined?(Gchart)

  handler_path = File.expand_path(File.dirname(__FILE__))
  library_path = File.join(handler_path, 'vendor/googlecharts-1.6.12/lib/')
  $:.unshift library_path
  # Require the library
