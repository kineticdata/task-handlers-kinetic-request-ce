...
class KineticRequestCeUserUpdateV5
    ...
    if response.nil?
      ...
      if @parameters["append_or_replace_memberships"]=="Append"
        ...
        if !@parameters["memberships"].empty?
          ...
          new_memberships = JSON.parse(@parameters["memberships"])
          # Iterate through each new memberships
          new_memberships.each do |new|
            # Initialize a value to assume no match was found
            match = false
            new_team = new['team']['name']
            # Iterate through each current team
            current_memberships.each do |current|
              current_team = current['team']['name']
              # If the current team no longer exists in the space, remove it
              if teamList.include?(current_team)
                user['memberships'].delete_if {|attr| attr['team']['name'] == current_team}
              else
                # If the new team already exists continue
                if current_team == new_team
                  # Set flag to indicate the team exist in current list
                  match = true
                  break
                end
              end
            end
            # If the team didn't exist add it to the user.
            if !match
              user["memberships"].push(new)
            end
          end
        end
