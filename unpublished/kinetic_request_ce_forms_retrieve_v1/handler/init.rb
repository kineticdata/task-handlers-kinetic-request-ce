...
class KineticRequestCeFormsRetrieveV1
  ...
  def execute
    ...
    begin
      ...
      route = @info_values['server'] + '/' + space_slug + 
                    '/app/api/v1/kapps/' + @parameters['kapp_slug'] + '/forms/?include=details'
	...
	results = JSON.parse(response)["forms"]
	puts "results: #{results.to_json}" if @enable_debug_logging
	return <<-RESULTS
          <results>
            <result name="Forms">#{escape(results.to_json)}</result>
