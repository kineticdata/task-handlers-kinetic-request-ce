...
if not defined?(Sequel)
  ...
  handler_path = File.expand_path(File.dirname(__FILE__))
  # Calculate the location of our library and add it to the Ruby load path
  library_path = File.join(handler_path, "vendor/sequel-4.46.0/lib")
  $:.unshift library_path
  # Require the library
  require "sequel"
end
...
elsif Sequel::VERSION.to_s != "4.46.0"
  raise "Incompatible library version #{Sequel::VERSION} for Sequel.  Expecting version 4.46.0."
end
...
begin
  ...
  handler_path = File.expand_path(File.dirname(__FILE__))
  require File.join(handler_path, "vendor", "ojdbc7.jar")
