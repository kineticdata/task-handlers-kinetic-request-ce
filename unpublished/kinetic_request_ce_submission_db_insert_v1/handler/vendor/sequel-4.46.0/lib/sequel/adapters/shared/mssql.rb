...
module Sequel
  module MSSQL
    ...
    module DatabaseMethods
      ...
      # MSSQL prefers the varchar(max) datatype unless < MSSQL 2005
      def type_literal_generic_string(column)
        column[:unicode] = column[:unicode].nil? ? false : column[:unicode]
        if column[:fixed] && column[:unicode] == true
          "nchar(#{column[:size]||255})"
        elsif column[:fixed] && column[:unicode] == false
          "char(#{column[:size]||255})"
        elsif (column[:text] == false or column[:size]) && column[:unicode] == true
          "nvarchar(#{column[:size]||255})"
        elsif (column[:text] == false or column[:size]) && column[:unicode] == false
          "varchar(#{column[:size]||255})"
        elsif column[:text] && column[:size].nil? == true && column[:unicode] == false && dataset.send(:is_2005_or_later?)
          "varchar(max)"
        elsif column[:text] && column[:size].nil? == true && column[:unicode] == true && dataset.send(:is_2005_or_later?)
          "nvarchar(max)"
        else
          :text
        end
